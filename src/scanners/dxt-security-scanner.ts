  ...
  auditExtension(extName: string, ext: DxtExtensionConfig, filePath?: string): Finding[] {
    ...
    if (!sandboxed && externalData && localExec) {
      findings.push({
        ...
        severity: 'critical',
        title: 'DXT-001',
        description: `DXT extension "${extName}" is unsandboxed with external data source AND local executor — arbitrary code execution via malicious input (e.g. calendar invite)`,
        file: filePath,
    ...
    if (hasUnrestrictedPermission(perms, 'file_system')) {
      findings.push({
        ...
        severity: 'high',
        title: 'DXT-002',
        description: `DXT extension "${extName}" has unrestricted file system access`,
        file: filePath,
    ...
    if (hasUnrestrictedPermission(perms, 'network')) {
      findings.push({
        ...
        severity: 'high',
        title: 'DXT-003',
        description: `DXT extension "${extName}" has unrestricted network access`,
        file: filePath,
    ...
    if (hasUnrestrictedPermission(perms, 'code_execution')) {
      findings.push({
        ...
        severity: 'high',
        title: 'DXT-004',
        description: `DXT extension "${extName}" has code execution permission enabled`,
        file: filePath,
    ...
    if (!sandboxed) {
      findings.push({
        ...
        severity: 'high',
        title: 'DXT-005',
        description: `DXT extension "${extName}" is running without sandboxing`,
        file: filePath,
    ...
    if (isUnsigned(ext)) {
      findings.push({
        ...
        severity: 'high',
        title: 'DXT-006',
        description: `DXT extension "${extName}" is unsigned`,
        file: filePath,
    ...
    if (!isUnsigned(ext) && isUnverified(ext)) {
      findings.push({
        ...
        severity: 'medium',
        title: 'DXT-007',
        description: `DXT extension "${extName}" is signed but signature is not verified`,
        file: filePath,
    ...
    if (
      ...
      if (sandboxed || !externalData || !localExec) {
        findings.push({
          ...
          severity: 'medium',
          title: 'DXT-008',
          description: `DXT extension "${extName}" has both file system and network access without sandboxing — potential exfiltration vector`,
          file: filePath,
    ...
    if (!sandboxed && externalData && !localExec) {
      findings.push({
        ...
        severity: 'medium',
        title: 'DXT-009',
        description: `DXT extension "${extName}" has external data sources without sandboxing`,
        file: filePath,
    ...
    if (ext.executors?.some((ex) => ex.unrestricted === true)) {
      findings.push({
        ...
        severity: 'medium',
        title: 'DXT-010',
        description: `DXT extension "${extName}" has an unrestricted executor`,
        file: filePath,
