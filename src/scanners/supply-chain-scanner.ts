    ...
    while ((match = regex.exec(file.content)) !== null) {
      ...
      if (SUSPICIOUS_DECODED.test(decoded)) {
        findings.push({
          ...
          rule: 'SUPPLY-001',
          severity: 'high',
          title: 'Suspicious Base64-encoded command detected',
          description: 'Suspicious Base64-encoded command detected',
    ...
    for (const { pattern, desc } of RCE_PATTERNS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        findings.push({
          ...
          rule: 'SUPPLY-002',
          severity: 'critical',
          title: `Remote code execution: ${desc}`,
          description: `Remote code execution: ${desc}`,
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
      ...
      for (const ip of ioc.malicious_ips) {
        ...
        while (idx !== -1) {
          findings.push({
            ...
            rule: 'SUPPLY-003',
            severity: 'critical',
            title: `Known malicious IP detected: ${ip}`,
            description: `Known malicious IP detected: ${ip}`,
            file: file.relativePath,
      ...
      for (const domain of ioc.malicious_domains) {
        ...
        while (idx !== -1) {
          findings.push({
            ...
            rule: 'SUPPLY-003',
            severity: 'critical',
            title: `Known malicious domain detected: ${domain}`,
            description: `Known malicious domain detected: ${domain}`,
            file: file.relativePath,
            line: findLineNumber(file.content, idx),
    ...
    for (const { pattern, desc } of CREDENTIAL_PATTERNS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        findings.push({
          ...
          rule: 'SUPPLY-004',
          severity: 'critical',
          title: `Credential theft: ${desc}`,
          description: `Credential theft: ${desc}`,
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
    ...
    for (const { pattern, desc } of EXFIL_PATTERNS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        findings.push({
          ...
          description: `Data exfiltration: ${desc}`,
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
    ...
    for (const { pattern, desc } of PERSISTENCE_PATTERNS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        findings.push({
          ...
          title: `Persistence mechanism: ${desc}${isDocFile ? ' [documentation reference]' : ''}`,
          description: `Persistence mechanism: ${desc}${isDocFile ? ' [documentation reference]' : ''}`,
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (reason) {
        findings.push({
          ...
          rule: 'SUPPLY-007',
          severity: 'critical',
          title: `Known malicious/typosquatted Python package: ${pkgName} (${reason})`,
          description: `Known malicious/typosquatted Python package: ${pkgName} (${reason})`,
          file: file.relativePath,
          line: i + 1,
        ...
        for (const m of pkgMatches) {
          ...
          if (reason) {
            findings.push({
              ...
              rule: 'SUPPLY-007',
              severity: 'critical',
              title: `Known malicious/typosquatted Python package: ${rawName} (${reason})`,
              description: `Known malicious/typosquatted Python package: ${rawName} (${reason})`,
              file: file.relativePath,
              line: i + 1,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while ((match = regex.exec(trimmed)) !== null) {
        findings.push({
          ...
          rule: 'SUPPLY-008',
          severity: 'high',
          title: 'Suspicious URL-based Python dependency (non-trusted host)',
          description: 'Suspicious URL-based Python dependency (non-trusted host)',
          file: file.relativePath,
          line: i + 1,
    ...
    for (const { pattern, desc } of SETUP_PY_DANGEROUS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        findings.push({
          ...
          rule: 'SUPPLY-009',
          severity: 'critical',
          title: `Dangerous setup.py pattern: ${desc}`,
          description: `Dangerous setup.py pattern: ${desc}`,
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
