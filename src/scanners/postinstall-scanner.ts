    ...
    if (!fs.existsSync(nodeModulesPath)) {
      return {
        ...
        findings: [{
          scanner: this.name,
          severity: 'info',
          title: 'No node_modules directory found',
    ...
    try {
      ...
      for (const hook of LIFECYCLE_HOOKS) {
        ...
        findings.push({
          ...
          file: relPath,
          title: `Package "${pkg.name || 'unknown'}" has ${hook} script`,
          description: `Package "${pkg.name || 'unknown'}" has ${hook} script`,
        ...
        for (const { pattern, desc, severity } of SUSPICIOUS_PATTERNS) {
          if (pattern.test(script)) {
            findings.push({
              ...
              file: relPath,
              title: `Suspicious ${hook} script in "${pkg.name || 'unknown'}": ${desc}`,
              description: `Suspicious ${hook} script in "${pkg.name || 'unknown'}": ${desc}`,
  ...
  private checkForMaliciousIndicators(
    ...
    if (suspiciousTLDs.test(script)) {
      findings.push({
        ...
        file,
        title: `Suspicious TLD in ${hook} script of "${packageName}"`,
        description: `Suspicious TLD in ${hook} script of "${packageName}"`,
    ...
    if (ipMatches && ipMatches.some(ip => !ip.startsWith('127.') && !ip.startsWith('0.'))) {
      findings.push({
        ...
        file,
        title: `IP address found in ${hook} script of "${packageName}"`,
        description: `IP address found in ${hook} script of "${packageName}"`,
    ...
    if (base64Pattern.test(script) && /base64|atob|Buffer\.from/.test(script)) {
      findings.push({
        ...
        file,
        title: `Base64-encoded content in ${hook} script of "${packageName}"`,
        description: `Base64-encoded content in ${hook} script of "${packageName}"`,
