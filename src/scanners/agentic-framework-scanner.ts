  ...
  check(file: FileEntry): Finding[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      findings.push({
        ...
        rule: 'AGENTIC-001',
        severity,
        title: 'LangChain API key found in .env file',
        description: 'LangChain API key found in .env file',
  ...
  check(file: FileEntry): Finding[] {
    ...
    for (const { key, value, path: fieldPath } of flagged) {
      ...
      findings.push({
        ...
        severity: 'high',
        file: file.relativePath,
        title: `LangChain config contains plaintext API key: ${key}`,
        description: `LangChain config contains plaintext API key: ${key}`,
      ...
      if (file.relativePath.endsWith('.json')) {
        ...
        for (const { key, value, path: fieldPath } of extractFieldValues(parsed003, apiKeyFields)) {
          ...
          findings.push({
            ...
            severity: 'critical',
            file: file.relativePath,
            title: `AutoGen config contains plaintext API key: ${key}`,
            description: `AutoGen config contains plaintext API key: ${key}`,
      ...
      if (/\.ya?ml$/i.test(file.relativePath)) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          findings.push({
            ...
            file: file.relativePath,
            line: i + 1,
            title: 'AutoGen YAML config contains plaintext api_key',
            description: 'AutoGen YAML config contains plaintext api_key',
    ...
    if (isAutogenPython) {
      ...
      while ((match = pattern.exec(file.content)) !== null) {
        ...
        findings.push({
          ...
          title: 'AutoGen Python config contains hardcoded api_key',
          description: 'AutoGen Python config contains hardcoded api_key',
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const pat of keyFieldPatterns) {
          ...
          findings.push({
            ...
            file: file.relativePath,
            line: i + 1,
            title: `CrewAI config contains plaintext tool API key: ${m[1]}`,
            description: `CrewAI config contains plaintext tool API key: ${m[1]}`,
      ...
      for (let i = 0; i < lines.length; i++) {
        for (const pat of keyFieldPatterns) {
          ...
          findings.push({
            ...
            file: file.relativePath,
            line: i + 1,
            title: `CrewAI Python file contains hardcoded tool API key: ${m[1]}`,
            description: `CrewAI Python file contains hardcoded tool API key: ${m[1]}`,
    ...
    for (const { pattern, desc } of HARDCODED_KEY_PATTERNS) {
      ...
      while ((match = regex.exec(file.content)) !== null) {
        ...
        findings.push({
          ...
          file: file.relativePath,
          line: findLineNumber(file.content, match.index),
          title: `Hardcoded ${desc} detected in agentic framework code`,
          description: `Hardcoded ${desc} detected in agentic framework code`,
